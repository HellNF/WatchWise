...
const PrismaticBurst = ({
  ...
  useEffect(() => {
    ...
    const dprValue = Math.min(window.devicePixelRatio || 1, dpr);
    const renderer = new Renderer({ dpr: dprValue, alpha: false, antialias: false });
    ...
    const update = (now: number) => {
      ...
      if (frameInterval > 0 && nowSec - lastRender < frameInterval) {
        raf = requestAnimationFrame(update);
        return;
      }
      lastRender = nowSec;
