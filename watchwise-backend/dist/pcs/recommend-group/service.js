...
const scoring_1 = require("../recommend-user/scoring");
const enrich_1 = require("../recommend-user/enrich");
const explain_1 = require("../recommend-user/explain");
const serendipity_1 = require("../recommend-user/serendipity");
...
async function recommendForGroup(groupId, requesterId, context, options) {
    ...
    if (options?.sessionId) {
        ...
        if (!sessionReady) {
            throw new errors_1.AppError("INVALID_INPUT", 409, "Group session not ready");
        }
    }
    const mergedContext = {
        ...sessionContext,
    ...
    if (!finalRanked.length) {
        throw new errors_1.AppError("NOT_FOUND", 404, "No recommendations available");
    }
    const compatibilities = await computeCompatibilityScores(filteredCandidates, mergedContext, memberProfiles);
    const scoredWithCompatibility = finalRanked.map((entry) => {
...
async function computeCompatibilityScores(candidates, context, memberProfiles) {
    if (!memberProfiles.length || !candidates.length) {
        return new Map();
    }
    const perUserScores = await Promise.all(memberProfiles.map((profile) => (0, scoring_1.scoreMovies)(candidates, context, profile)));
...
async function isSessionReady(group, session) {
    if (!group || !session)
        return false;
