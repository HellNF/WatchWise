...
exports.computeProfileConfidence = computeProfileConfidence;
...
function aggregateGroupPreferenceProfile(profiles) {
    ...
    for (const key of PROFILE_KEYS) {
        ...
        const allValues = new Set(profiles.flatMap((p) => Object.keys(p[key] ?? {})));
...
function simpleMeanAggregate(profiles) {
    const merged = emptyProfile();
